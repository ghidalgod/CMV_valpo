...
class Ion_auth
{
	 ...
	 * @author Mathew
	 */
	public function register($identity, $password,$center, $email, $additional_data = array(), $group_ids = array())
	{
		$this->ion_auth_model->trigger_events('pre_account_creation');

		$email_activation = $this->config->item('email_activation', 'ion_auth');

		$id = $this->ion_auth_model->register($identity, $password, $email, $additional_data, $group_ids, $center);
	...
	public function redirectLoginIn() {
		if (!$this->logged_in())
        {
            redirect(site_url('auth'), 'location', 301);
        }
	}
	...
	public function loginIn() {
		if (!$this->logged_in())
        {
            return $this->message_handler->get('Warning', 'AuthInactiveSessionSession');
        }
        return $this->message_handler->get('Successful', 'AuthActiveSession');
	}
	
	public function getCodigosIgestion(){
		return $this->ion_auth_model->codigos_igestion();
	}
}
