...
class Ion_auth_model extends CI_Model
{
	...
	public function get_users_centers($id){
		$this->db->from('users_igestion');
        $this->db->where('users_igestion.user_id',$id);
        $query = $this->db->get();
        if ($query->num_rows()>0) {
            $data = $query->result_array();
            foreach ($data as $key => $cod) {
            	$values[$key] = $data[$key]['codigo'];
            }
        }
        if(empty($values)){
        	return null;
	 ...
	 * @author    Mathew
	 */
	public function register($identity, $password, $email,$centers, $additional_data = array(), $groups = array())
	{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, first_name, last_name, phone, company')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				//*********Add Establecimiento at user form Process Maker DB
				$this->set_session($user);
				if($this->ion_auth->in_group(5)){
				    $this->load->model('ProcessMaker_model');
		            $this->ProcessMaker_model->setEstablecimiento();
				}
				//*********Process Maker*******************
	...
	public function add_center($center_ids, $user_id = FALSE){
		// if no id was passed use the current users id
		$user_id || $user_id = $this->session->userdata('user_id');

		if(!is_array($center_ids)){
			$center_ids = array($center_ids);
		}
		...
		foreach ($center_ids as $center_id){
			...
			$this->db->insert('users_igestion', $data);
	...
	public function codigos_igestion() {
		
		$this->trigger_events('codigos_igestion');
	...
	public function remove_from_igestion($id) {
		// user id is required
		if (empty($id)){
			return FALSE;
		}else{
			$this->db->delete('users_igestion', array('user_id' => $id));
			$return = TRUE;
		}
		return $return;
	}
	...
	public function login_remembered_user() {
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', id, email, last_login, first_name, last_name, phone')
