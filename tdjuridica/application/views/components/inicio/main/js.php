        ...
        "columnDefs": [
            ...
            { 
                ...
                "render": function ( data, type, row ) {
                   ...
                   /*     var fecha1 = moment(row.fecha_not);
                        var fecha2 = moment(row.plazo_fiscal);
                        $contador = $fecha->diff($fecha2);
                         */
            ...
            },
            {
                ...
                "render": function ( data, type, row ) {
                    ...
                    if(row.etapa == 3) return 'Sobreseimiento';       //DEBE SER APROBADA O RECHAZADA POR EL SECRETARIO GENERAL
                    if(row.etapa == 4) return 'Formulación de cargos';  // ABSOLUCIÓN O SANCIÓN   
                    if(row.etapa == 5) return 'Dictamen Sumario Fiscal';                    
                    if(row.etapa == 6) return 'Resolución Final';
                        return '<span class="label label-success">'+ 'En Espera' + '</span>';                                
                }
            },
            {
                ...
                "render": function ( data, type, row ) {
                    ...
                    if (row.etapa == 3 && row.sentencia == 3) return '<span class="label label-info">' + 'SOBRESEIMIENTO'+'</span>';
                    if (row.etapa == 3 && row.sentencia == 4) return '<span class="label label-info">' + 'Formulación de cargos'+'</span>';
                    return '<span class="label label-success">'+ 'En Espera' + '</span>'; 
                }
            },
          {
                ...
                "render": function ( data, type, row ) {
                    ...
                    if(row.etapa == 1 ) link_edit = '<?= site_url('Proc/SumarioSalud/mostrarSumario') ?>/' + row.id;
                    ...
                    if(row.etapa == 4) link_edit = '<?= site_url('Proc/SumarioSalud/mostrarSumario') ?>/' + row.id;
            ...
            },
            {
                ...
                "render": function ( data, type, row ) {
                    var link_edit = '*';
                    link_edit = '<?= site_url('Flujoprocedimientos/Sumario_salud/enviarNotificacion') ?>/' + row.id;
                    var options_normal = '<div class="hidden-sm hidden-xs action-buttons">';
                    var edit_normal = '<a class="blue" href="' + link_edit + '" title="Continuar"><i class="ace-icon fa fa-envelope bigger-130"></i></a>';
                    options_normal += edit_normal;
                    options_normal += '</div>';
